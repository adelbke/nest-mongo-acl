...
describe('Assess The Schema setup', () => {
    ...
    let ResourceModel: ReturnModelType<typeof ResourceWithAcl & AclMethodsCls, IAclQueryHelpers>;
    ...
    beforeAll(async () => {
        ...
        ResourceModel = getModelForClass(ResourceWithAcl) as ReturnModelType<typeof ResourceWithAcl & AclMethodsCls, IAclQueryHelpers>;
    ...
    test(`Expect user to group usage to not disturb the functioning of the schema`, async () => {
        ...
        expect(
            entry.hasAccess(adminUser, 'read')
        ).toEqual(false);

        expect(
            entry.hasAccess(regularUser, 'read')
        ).toEqual(true);


        expect(
            entry.hasAccess(adminUser, 'write')
        ).toEqual(true);

        expect(
            entry.hasAccess(regularUser, 'write')
        ).toEqual(false);
    })
})